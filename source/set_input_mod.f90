...
!              deltaE(0) = Ldiffuse/nPhotonsDiffuse
        ...
        if (lg1D) then
           ...
           stop
        else if (resLinesTransfer >= minConvergence .and. resLinesTransfer /= 101.) then
           ...
           stop
        else if (lgPlaneIonization .and. (nxin(1) <=2 .or. nzin(1) <=2)) then
           print*, " readInput: plane parallel simulation must have cuboid sides at least 3 cells deep&
                &- please increase nx and/or nz in the mother grid to at least 3-",nxin(1), nzin(1)
           stop
        ...
        else if (resLinesTransfer <= 100 .and. .not.lgGas) then
           print*, "! readInput: [warning] gas processes must be activated, to use the&
...
!           deltaE(0) = Ldiffuse/nPhotonsDiffuse
      ...
      subroutine setFluorescenceInput()
        ...
        print*, 'in setFluorescenceInput'

        fluoLabArray = (/'FeKa','FeL1', 'FeL2',' CKa',' NKa',' OKa','NeKa',&
             & 'MgKa','AlKa','SiKa',' SKa','ArKa','CaKa','TiKa'/)

        filename = 'input/fluorescence.in'
        ...
        read(10,*) nFluo, nVP, nPacketsFluo
        read(10,*) fluoCubeMineV, fluoCubeMaxeV        
        print*,  nFluo, nVP, nPacketsFluo, ' nFluo, nVP, nPacketsFluo'
        print*, fluoCubeMineV, fluoCubeMaxeV, ' fluoCubeMineV, fluoCubeMaxeV'

        allocate (fluorescenceLabel(1:nfluo), stat=err) 
        if (err /= 0) then
           print*, '! setFluorescenceInput: allocation error for fluorescenceLabel pointer'
           stop
        end if
        allocate (fluorescenceVP(1:2,1:nVP), stat=err) 
        if (err /= 0) then
           print*, '! setFluorescenceInput: allocation error for fluorescenceVP pointer'
           stop
        end if

        read(10,*) (fluorescenceLabel(i), i = 1, nfluo)
        print*, fluorescenceLabel, ' fluorescenceLabel(i)'

        do i = 1, nVP
           read(10,*) fluorescenceVP(1,i), fluorescenceVP(2,i)
           print*,fluorescenceVP(1,i), fluorescenceVP(2,i), ' fluorescenceVP(1,i), fluorescenceVP(2,i)'
        end do
      ...
      end subroutine setFluorescenceInput
