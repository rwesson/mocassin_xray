...
module emission_mod
    ...
    real (kind=8), dimension(34)        :: HeIRecLines    ! emissivity from HeI rec lines
    ...
    subroutine emissionDriver(grids, cellPloc, ig)
        implicit none

        real (kind=8) :: gammaHI(nbins)          ! HI fb+ff emission coefficient [e-40 erg*cm^3/s/Hz]
        real (kind=8) :: gammaHeI(nbins)         ! HeI fb+ff emission coefficient [e-40erg*cm^3/s/Hz]
        real (kind=8) :: gammaHeII(nbins)        ! HeII fb+ff emission coefficient [e-40erg*cm^3/s/Hz]
        real (kind=8) :: gammaHeavies(nbins)     ! Heavy ions f-b emission coefficient [e-40 erg*cm^3/s/Hz]
        real (kind=8) :: ffCoeff1(nbins)         ! ff emission coefficient for Z=1 ions [e-40 erg*cm^3/s/Hz]
        real (kind=8) :: ffCoeff2(nbins)         ! ff emission coefficient for Z=2 ions [e-40 erg*cm^3/s/Hz]
        real (kind=8) :: twoPhotHI(nbins)        ! HI 2photon emission coefficient [e-40 erg*cm^3/s/Hz]
        real (kind=8) :: twoPhotHeI(nbins)       ! HeI 2photon emission coefficient [e-40 erg*cm^3/s/Hz]
        real (kind=8) :: twoPhotHeII(nbins)      ! HeII 2photon emission coefficient [e-40 erg*cm^3/s/Hz]

        ! units are [e-40erg/cm^3/s/Hz]
        real (kind=8) :: emissionHI(nbins)               ! HI continuum emission coefficient
        real (kind=8) :: emissionHeI(nbins)              ! HeI continuum emission coefficient
        ...
        integer, intent(in) :: cellPloc, ig  ! pointers to this cell
...
!        cellPUsed = grids(iG)%active(ix, iy, iz)
        ...
        emissionHI = (gammaHI + twoPhotHI) * grids(iG)%elemAbun(abFileUsed,1) * ionDenUsed(elementXref(1),2)*NeUsed + gammaHeavies*NeUsed
        emissionHeI = (gammaHeI + twoPhotHeI ) * grids(iG)%elemAbun(abFileUsed,2) * ionDenUsed(elementXref(2),2)*NeUsed
        emissionHeII= (gammaHeII + twoPhotHeII ) * grids(iG)%elemAbun(abFileUsed,2) * ionDenUsed(elementXref(2),3)*NeUsed
...
!          dV = getVolume(grids(iG),cellPUsed)
        ...
        if (lgGas.and.convPercent>=resLinesTransfer .and. (.not.lgResLinesFirst) &
             & .and. (.not.lgVoronoi) .and. (.not.nIterateMC==1)) then
           call setResLineEscapeProb(grids,ig,cellPused)
        end if
    ...
    contains
      ...
      subroutine initResLinePackets()
           ...
           do imul = 1, resLine(iRes)%nmul
              ...
              if (resLine(iRes)%elem==1) then
                 if ( resLine(iRes)%ion == 1 .and. &
                    ...
                    lineIntensity = lineIntensity+10**(-0.897*log10(TeUsed) &
                         &+ 5.05)* &
                         & grids(iG)%elemAbun(abFileUsed,1)*&
                         &ionDenUsed(elementXref(1),2)*&
                         &  NeUsed*grids(iG)%Hden(cellPUsed)*dV*&
                         & (1.-grids(iG)%fEscapeResPhotons(cellPUsed, &
                         &iRes))
              ...
              else if (resLine(iRes)%elem>1 .and. resLine(iRes)%elem == resLine(iRes)%ion) then

                 lineIntensity = lineIntensity+hydrolines(resLine(iRes)%elem,&
                 ...
                 if (forbiddenLines(resLine(iRes)%elem,resLine(iRes)%ion,&
                      &resLine(iRes)%moclow(imul),resLine(iRes)%mochigh(imul))> 1.e-35) &
        ...
        ! calculate number of continuum emitted extra packets to
        ! be transfered from this location
        ! NOTE :1.e-16 from 1.e45 (from dV) * 1.e-36 (from Lstar)
        !* 1.e-25 (from local forlines)
        grids(iG)%resLinePackets(cellPUsed) = &
             &nint(lineIntensity*1.e-16/(Lstar(1)/Nphotons(1)))
      ...
      subroutine fb_ff()
        ...
        real (kind=8)                :: aFit, &    ! general calculations factors
             & factor, &
             & expFactor
        real (kind=8)                :: constant   ! general calculation constant
        real (kind=8)                :: phXSecHI,& ! phot x sec for HI
                ...
                do ion = 1, nstages-1
                    ...
                    if ( (i >= IPnuP) .and. (i < highNuP) ) then
                        ...
                        gammaHeavies(i) = gammaHeavies(i) + &
                             ...
                             &grids(iG)%elemAbun(abFileUsed,elem)
    ...
    function hydro2phot(nu, Z)
        ...
        real (kind=8)    :: hydro2phot   ! emission coeff for 2-phot cont [e-40 erg/cm^3/s/Hz]
...
&                 0.88*((y*(1-y))**1.53)*(4*y*(1-y))**0.8)
        ...
        case (2) ! HeII
            ...
            if (TeUsed <= 5000.) then
                ...
                alphaEffHeII21S = 10**( log10(6.161) +&
                     & factor*log10(TeUsed/5000.) )
            else if (TeUsed > 10000. .and. TeUsed <= 15000. ) then
                factor = log10(3.189/4.091) / log10(1.5)
                alphaEffHeII21S = 10**( log10(4.091) + &
                     &factor*log10(TeUsed/10000.) )
            else if (TeUsed > 15000. .and. TeUsed < 30000. ) then
                factor = log10(2.035/3.189) / log10(2.)
                alphaEffHeII21S = 10**( log10(3.189) + &
                     &factor*log10(TeUsed/15000.) )
            end if
    ...
    subroutine HeI2photSub()
        ...
        real                :: alphaEff21SHeI! effective rec coeff to
        ! HeI 21S [e-14 cm^3/s]
                                             ! Almog & Netzer, MNRAS 238(1989)57
...
!        ffCoeff2 = fourPi*54.43*2*2*gauntFFHeII*exp(dble(-nuArray*hcRyd_k/TeUsed))/sqrTeUsed
    ...
    ! this function calculates the ff emission
    ! coeficient for interactions between
    ...
    subroutine RecLinesEmission()
        ...
        do izp = 1, elup
           if (lgElementOn(izp) .and. nstages > izp) then
              ...
              idenp = 1
              do iden = 1, 13
                 if (NeUsed.gt.hydrolinesloc(iden)%dens .and. hydrolinesloc(iden)%dens.gt. 0.) idenp = iden
              end do
              ...
              Hbeta(izp) = Hbeta(izp)*NeUsed*ionDenUsed(elementXref(izp),izp+1)*&
                   &grids(iG)%elemAbun(abFileUsed,izp)
...
!             & NeUsed*Lalpha
       ...
       elseif(denint==0) then
        ...
        HeIRecLines=HeIRecLines*NeUsed*grids(iG)%elemAbun(abFileUsed,2)*&
    ...
    subroutine RecLinesEmissionNew()
        ...
        do izp = 1, elUp
           if (lgElementOn(izp) .and. nstages > izp) then
              ...
              if ((hydrolinesloc(13)%dens>0. .and. idenp<13) .or. (hydrolinesloc(13)%dens==0. &
                   & .and. idenp<9)) then
                 ! interpolate
                 do iup = 2, 15
                    do ilow = 1, min(8,iup-1)
        ...
        do izp = elUp+1, nElements
           ...
           if (lgElementOn(izp) .and. nstages > izp) then
              ...
              hydrolinesloc=hydroLinesData(9,itemp,:)
...
!             & NeUsed*Lalpha
        ...
        if (denint>0.and.denint<3) then
           do i = 1, 34
              ...
              HeIRecLines(i) = x1+((x2-x1)*(NeUsed-100.**denint)/&
                ...
                if (.not. (HeIRecLines(i) >= 0. .and. HeIRecLines(i) <= 1.e30)) then
                   print*, '! recLinesEmission: insanity in HeIRecLines calculation [i, denint, T4, NeUsed, x1,x2]', i, &
                        & denint, T4, NeUsed, x1,x2
                   print*, ' HeIRecLineCoeff(i,denint,:) ', HeIRecLineCoeff(i,denint,:)
                   print*, ' HeIRecLineCoeff(i,denint+1,:) ', HeIRecLineCoeff(i,denint+1,:)
                   stop
                end if
       ...
       elseif(denint==0) then
           ...
           HeIRecLines(i) = HeIrecLineCoeff(i,1,1)*(T4**(HeIrecLineCoeff(i,1,2)))*exp(HeIrecLineCoeff(i,1,3)/T4)
             if (.not.(HeIRecLines(i) >= 0. .and. HeIRecLines(i) <= 1.e30)) then
               print*, '! recLinesEmission: insanity in HeIRecLines calculation [i, denint, T4, NeUsed]', i, denint, T4, NeUsed
               print*, ' HeIRecLineCoeff(i,1,:) ', HeIRecLineCoeff(i,1,:)
               stop
             end if
           end do
         elseif(denint==3) then
        ...
        do elem = 3, nElements
           do ion = 1, min(elem+1, nstages)
              ...
              if (lgDataAvailable(elem, ion)) then
                 ...
                 if (ion<min(elem+1,nstages) .and. &
                      &ionDenUsed(elementXref(elem),ion)>0.) then
                    call equilibrium(file_name=dataFile(elem, ion), &
                         ...
                         &Ne=NeUsed, FlineEm=forbiddenLines(elem, &
                         &ion,1:nForLevels, 1:nForLevels),rec=.true.)
                 else
                    call equilibrium(file_name=dataFile(elem, ion), &
                         &ionDenUp=0., Te=TeUsed, Ne=NeUsed, &
                         &FlineEm=forbiddenLines(elem, ion,&
                         &1:nForLevels,1:nForLevels),rec=.true.)
                 end if
    ...
    subroutine setDiffusePDF()
        ...
        real              :: sumDiffuseDust(nbins) ! summation terms for dust emission
        real              :: sumDiffuseHI(nbins)   ! summation terms for HI emission
        real              :: sumDiffuseHeI(nbins)  ! summation terms for HeI emission
        ...
        do i = 1, nbins
           ...
           if (.not.(normHI>=0. .and.  normHeI>= 0. .and. normHeII>=0.)) then
             print*, 'insanity in calculation of normHI, normHeI or normHeII'
             print*, 'i, normHI, normHeI, normHEII,I(i),  emissionHeI(i), emissionHeII(i)'
             print*, i, normHI, normHEI, normHeII, emissionHI(i),  emissionHeI(i), emissionHeI
             stop
           end if
        ...
        HeIILyman(4) = HeIILyman(4)*NeUsed*ionDenUsed(elementXref(2),3)*&
             &grids(iG)%elemAbun(abFileUsed,2)
        ...
        do i = 1, NHeIILyman
            ...
            ! ... and to HeII normalization constant
            normHeII = normHeII + HeIILyman(i)
            if (.not. HeIILyman(i) >= 0.) then
              print*, 'insanity in HeIILyman(i), i', HeIILyman(i), i
              stop
            end if
        end do
        ...
        if (lgDust) then
           ...
           do ai = 1, nSizes
              do nS = 1, nSpecies
                 ...
                 if (grids(iG)%Tdust(nS,ai,cellPUsed)<TdustSublime(nS)) then
                    ...
                    if (lgQHeat .and. grainRadius(ai)<minaQHeat .and. &
                       ...
                       if (lgWritePss .and. taskid==0) then
                          ...
                          write(89, *)  cellpUsed, ns, grainRadius(ai), tg
        ...
        if (.not.normalize >= 0. ) then
          print*, 'insanity in calculation of continuum normalize, normalize, normHI, normHeI, normHeII', normalize, &
          & normHI, normHeI, normHeII
          stop
        end if
...
&                           HeIRecLines(j) / grids(iG)%totalLines(cellPUsed)
        ...
        if (lgFluorescence) grids(iG)%totalEmission(cellPUsed) = normalize
      ...
      do n = 1, nSpecies
         do ai = 1, nSizes
            ...
            if (grids(iG)%Tdust(n,ai,cellPUsed) >0. .and. &
                 & grids(iG)%Tdust(n,ai,cellPused)<TdustSublime(n)) then
  ...
  subroutine equilibrium(file_name, ionDenUp, Te, Ne, fLineEm, wav, rec)
    implicit none

    real (kind=8), dimension(nForLevels,nForLevels), &
         & intent(out) :: fLineEm     ! forbiddn line emissivity
...
888 elem=i
    ...
    if (rec) then
       ...
       ! (direct + cascades)
       if (file_name=='data/ci.dat' .or. file_name=='data/ciii.dat' &
            & .or. file_name=='data/ni.dat' .or. file_name=='data/neiv.dat' &
            & .or. file_name=='data/oii.dat' .or. file_name=='data/oiii.dat' &
            & .or. file_name=='data/nii.dat' .or. file_name=='data/nev.dat' &
            !         & .or. file_name=='data/niv.dat' .and. lgRecContLowIonsOn ) then
    ...
    do i = 1, nLev-1
       do j = i+1, nLev
          if (atomic_data_array(elem,ion)%a(j,i) /= 0.d0) then
             ...
             if (j == atomic_data_array(elem,ion)%iup2p .and. i ==1 .and. atomic_data_array(elem,ion)%atp(j,i) >0.) then
  ...
  subroutine lured(a,n)
    ...
    real (kind=8),&
         & intent(inout), dimension(:,:)    :: a
    real (kind=8) :: factor                     ! general calculation factor
  ...
  subroutine initResLines(grid)
    ...
    nResLines =0
    do i = 1, safeLimit
!       read(unit=19,fmt='(A2,I2,A108,I4)',iostat=ios) label,ionlab,reader, nmul
       read(unit=19,fmt='(A2,I2,A43,F9.4,A58,I4)',iostat=ios) label,ionlab,reader,wread,reader, nmul
       ...
       wread = c*1.e8/(wread*fr1Ryd)
       if (lgElementOn(el) .and. ionlab<nstages .and. wread < nuMax) nResLines=nResLines+1
    ...
    do ii = 1, nResLinesFile
       ...
       read(unit=19,fmt='(A2,I2,1x,A23,11x,A7,1x,F9.4,7x,F9.6,4x,F12.6,1x,I2,1x,I2,1x,E8.2,1x,E8.2,1x,I2)', &
            ...
            & resLine(nL)%transition, resLine(nL)%multiplet, resLine(nL)%wav, &
            &resLine(nL)%Elow, resLine(nL)%Ehigh,&
            & resLine(nL)%glow, &
       ...
       wread = c*1.e8/(resLine(nL)%wav*fr1Ryd)

       if (lgElementOn(resLine(nL)%elem) .and. resLine(nL)%ion < nstages .and. wread < nuMax) then

          ! convert A to Ryd
!          resLine(nL)%freq = c*1.e8/(resLine(nL)%wav*fr1Ryd)
          resLine(nL)%freq  = wread
          ...
          deallocate (resLine(nL)%moclow)
          deallocate (resLine(nL)%mochigh)
    ...
    if (taskid == 0) then
       ...
       do i = 1, nResLines
          do j = 1, resLine(i)%nmul
             ...
             print*, resLine(i)%moclow(j), resLine(i)%mochigh(j), resLine(i)%nuP
          end do
       end do
    end if
  ...
  subroutine setResLineEscapeProb(grids, gPin, cellPin)
    implicit none
    ...
    integer, intent(in) :: cellPin,gPin ! incoming cell pointers
    integer             :: xp,yp,zp ! cell pointers
    integer             :: xpin,ypin,zpin ! cell pointers
    ...
    xp=grids(gPin)%activeR(1, cellP)
    yp=grids(gPin)%activeR(2, cellP)
    zp=grids(gPin)%activeR(3, cellP)
    gp=gPin
    ...
    do iLine = 1, nResLines
       ...
       compoP = grids(gPin)%abFileIndex(cellP)
       ...
       call lineCentreXSec(resLine(iLine), TeUsed, a0)
       a0 = a0/1.e-14
       ...
       if (lgTest) then
          ...
          ! the above was changed in Version 3.00 in order to include continuum
          ! opacity as an extra source of line destruction
          if (lgDust) then
             k_d = grids(gPin)%opacity(cellP,resLine(iLine)%nuP)-&
                  &grids(gPin)%scaOpac(cellP,resLine(iLine)%nuP)
          else
             k_d = grids(gPin)%opacity(cellP,resLine(iLine)%nuP)
          end if
          ...
          do jdir = 0,ndiry
             ...
             do kdir = 0,ndirz
                ...
                do icount = 1, safeLimit
                   ...
                   if (cellP > 0 ) then
                      ...
                      if (xp == xpin .and. yp == ypin .and. zp == zpin .and. gP == gpin) then
                         ...
                         if (lgTest) then
                            ...
                            gasdensity = grids(gP)%Ne(grids(gP)%active(xp,yp,zp))
                   ...
                   if (cellP>0) then
                      if (lgTest) then
                         tau_mu  = tau_mu + a0*1.e-14*dS*gasdensity*&
                              &grids(gP)%elemAbun(compoP,resLine(iLine)%elem)*&
                              &ionFracTest
                      else
                         tau_mu  = tau_mu + a0*1.e-14*dS*gasdensity*&
                              & grids(gP)%elemAbun(compoP,resLine(iLine)%elem)*&
                              &grids(gP)%ionDen(cellP,elementXref(resLine(iLine)%elem),&
                              &resLine(iLine)%ion)
                      end if
                   end if
                      ...
                      if (uHat%x>0.) then
                         if ( xPmother < grids(gPmother)%nx ) then
                            ...
                            if ( rVec%x > grids(gPmother)%xAxis(xPmother)) then
                               print*, '! pathSegment: insanity occurred at&
                                    & mother grid transfer (x axis +)', &
                                    & rVec%x, gP, gPmother
                               stop
                            end if
                         end if
                ...
                if (tau_mu <= 0.) &
                     ...
                     & tau_mu, uHat, xPin,yPin,zPin, cellP, xp,yp,zp, a0, iLine
    ...
    !
    subroutine lineCentreXSec2(line, Tin, line_xSec)
      implicit none


      real, intent(inout)         :: Tin
      real, intent(out)        :: line_xSec
      type(resLine_type), intent(inout) :: line

      Tin = Tin/1.e6

      line_xSec = 9.31e-18*line%fik*line%wav*&
           &((aweight(line%elem)*Tin)**(1./2.))

    end subroutine lineCentreXSec2
