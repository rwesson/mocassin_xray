...
module elements_mod
    ...
    subroutine makeCollIonData()
      ...
      close(12)
      open(unit = 12, file = PREFIX//'/share/mocassinX/data/cion.dat', status='old', position='rewind')
    ...
    subroutine makeAugerData()
      ...
      close(12)
      open(unit = 12, file = PREFIX//'/share/mocassinX/data/auger.dat', status='old', position='rewind')
    ...
    subroutine getOuterShell(z, nElec, outShell, g0, g1)
      ...
      subroutine readHeIRecLines()
       ...
       close(13)
       open(file=PREFIX//'/share/mocassinX/data/HeIrecLines.dat', unit = 13, status='old')
    ...
    function limitShell(ion, shell, nElem)
      ...
      subroutine initRRDR()
        ...
        open(file=PREFIX//'/share/mocassinX/data/rrBadnell.dat', action="read", unit=19, status='old', position='rewind', iostat=ios)
        if (ios<0) then
        ...
        open(file=PREFIX//'/share/mocassinX/data/drBadnell.dat', action="read", unit=20, status='old', position='rewind', iostat=ios)
        if (ios<0) then
     ...
     subroutine makeElements()
       ...
       integer           :: i, iup, ilow, jj ! counters
       ...
       close(17)
       open(file=PREFIX//'/share/mocassinX/data/fileNames.dat', action="read", unit=17, status='old', position='rewind', iostat=ios)
       ...
       do elem = 3, nElements
          do ion = 1, min(elem+1, 31)
             ...
             filename = trim(PREFIX)//'/share/mocassinX/'//dataFile(elem,ion)
...
!read # levels and temps, then allocate arrays
                   ...
                   atomic_data_array(elem,ion)%iup2p = -1
               ...
               !             print*, (qom(k,indexlow(2),indexup(2)), k = 1, nTemp)
                      ...
                      do k =1 , 2502
                         ...
                         if (i>0 .and. j>0) then
                            ...
                            atomic_data_array(elem,ion)%atp(i,j) = ax
                         end if
                         !be
                      ...
                      do k = 1, atomic_data_array(elem,ion)%NLEVS-1
                         do l = k+1, atomic_data_array(elem,ion)%NLEVS
                            ...
                            if (i < 0 .and. j <0) then
                               atomic_data_array(elem,ion)%iup2p = abs(j)
                               i = abs(i)
                               j = abs(j)
                               atomic_data_array(elem,ion)%atp(j,i) = ax
                               if ( atomic_data_array(elem,ion)%a(j,i) > 0.) then
                                  jj = jj+1
                               end if
                            ...
                            elseif (i>0 .and. j>0) then
                               ...
                               print*, '! equilibrium: insane transition probability ', filename, i,j
                               stop
                            end if
!                            atomic_data_array(elem,ion)%a(j,i) = ax
                         end do
               ...
               !       read(unit=121,fmt=*,iostat=ios) a_fit, b_fit
                      ...
                      &atomic_data_array(elem,ion)%a_r(:),atomic_data_array(elem,ion)%a_d(:) !a_fit, b_fit
                      if (ios<0) then
                         exit
                      else
